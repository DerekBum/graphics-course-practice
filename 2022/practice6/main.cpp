...
void main()
{
    ...
    if (mode == 2) {
        newcoord = texcoord + vec2(sin(texcoord.y * 50.0 + time) * 0.01, 0.0);
    }
    vec4 albedo = texture(render_result, newcoord);
    out_color = vec4(albedo);
    if (mode == 1) {
        out_color[0] = floor(out_color[0] * 4.0) / 3.0;
        out_color[1] = floor(out_color[1] * 4.0) / 3.0;
        out_color[2] = floor(out_color[2] * 4.0) / 3.0;
    }
    if (mode == 3) {
        vec4 sum = vec4(0.0);
        float sum_w = 0.0;
    ...
    while (running)
    {
        ...
        for (int i = 0; i < 4; i++) {
            ...
            if (i == 0) {
                ...
                model = glm::rotate(model, model_angle, {1.f, 0.f, 0.f});
                projection = glm::ortho(-1.f, 1.f, -1.f / aspect_ratio, 1.f / aspect_ratio, near, far);
            }
            else {
                model = glm::rotate(model, model_angle, {0.f, 0.f, 1.f});
                projection = glm::ortho(-1.f, 1.f, -1.f / aspect_ratio, 1.f / aspect_ratio, near, far);
            }
