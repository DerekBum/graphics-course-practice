...
void main()
{
    ...
    vec3 point_direction = normalize(point_light_position - position);
    float point_distance = length(point_light_position - position);
    vec3 color = ambient +
        (diffuse(sun_direction) + specular(sun_direction)) * sun_color +
        (diffuse(point_direction) + specular(point_direction)) * point_light_color *
        (1.0 / (point_light_attenuation.x + point_light_attenuation.y * point_distance + point_light_attenuation.z * point_distance * point_distance));
...
int main() try {
    ...
    while (running) {
        ...
        view = glm::translate(view, {-camera_x, 0.f, 1.f});
        ...
        glUniform3f(sun_direction_location, sqrt(1.f / 3.f), sqrt(1.f / 3.f), sqrt(1.f / 3.f));
        glUniform3f(sun_color_location, 1.f, 0.9f, 0.8f);
        glUniform3f(point_light_position_location, sin(time) / 3.f, cos(time) / 3.f, sin(time) / 3.f);
        glUniform3f(point_light_color_location, 1.f, 0.f, 0.f);
        glUniform3f(point_light_attenuation_location, 1.f, 0.f, 0.01f);

        for (int i = 0; i < 3; i++) {
            for (int j = 0; j < 3; j++) {
                model = glm::translate(glm::mat4(1.f), {i - 1.f, j - 1.f, 0.f});
                model = glm::scale(model, {0.2f, 0.2f, 0.2f});
                glUniformMatrix4fv(model_location, 1, GL_FALSE, reinterpret_cast<float *>(&model));
                float roughness = (i + j * 3) / 16.f + 0.1f;
                glUniform1f(roughness_location, roughness);
                glUniform1f(glossiness_location, 1.f / roughness);
                glBindVertexArray(suzanne_vao);
